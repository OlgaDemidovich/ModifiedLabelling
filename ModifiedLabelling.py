...
parser.add_argument('--format', default='yolo', type=str,
                    choices=['yolo', 'voc'], help="Bounding box format")
parser.add_argument('--sort', action='store_true',
                    help="If true, shows images in order.")
parser.add_argument('--cross-thickness', default='1', type=int,
                    help="Cross thickness")
parser.add_argument('--bbox-thickness', default=bbox_thickness, type=int,
...
def voc_format(class_index, point_1, point_2):
    ...
    xmin, ymin = min(point_1[0], point_2[0]) + 1, min(point_1[1],
                                                      point_2[1]) + 1
    xmax, ymax = max(point_1[0], point_2[0]) + 1, max(point_1[1],
                                                      point_2[1]) + 1
...
def draw_text(tmp_img, text, center, color, size):
    ...
    cv2.putText(tmp_img, text, center, font, 0.7, color, size,
...
def draw_bboxes_from_file(tmp_img, txt_path, width, height):
    ...
    if os.path.isfile(txt_path):
        ...
        for line in content:
            ...
            if args.format == 'yolo':
                class_index, x_center, y_center, x_width, y_height = map(float,
                                                                         values_str)
                class_index = int(class_index)
                # convert yolo to points
                x1, y1, x2, y2 = yolo_to_x_y(x_center, y_center, x_width,
                                             y_height, width, height)
            ...
            cv2.rectangle(tmp_img, (x1, y1), (x2, y2), color,
                          thickness=args.bbox_thickness)
            tmp_img = draw_text(tmp_img, class_list[class_index], (x1, y1 - 5),
                                color, args.bbox_thickness)
    return tmp_img
...
def mouse_listener(event, x, y, flags, param):
    ...
    # AlexeyGy change: delete via right-click
        ...
        if point_1[0] == -1:
            ...
            # !!!!!!!!!!!!!!!!!!!!
            threshold = 5
            if abs(x - point_1[0]) > threshold or abs(
                    y - point_1[1]) > threshold:
                # second click
                point_2 = (x, y)
...
# selected image
TRACKBAR_IMG = 'Image'
cv2.createTrackbar(TRACKBAR_IMG, WINDOW_NAME, 0, last_img_index,
                   change_img_index)
...
while True:
    ...
    if point_1[0] != -1:
        color = class_rgb[class_index].tolist()
        # draw partial bbox
        cv2.rectangle(tmp_img, point_1, (mouse_x, mouse_y), color,
                      thickness=args.bbox_thickness)
        # if second click
        if point_2[0] != -1:
            ...
            if WITH_QT:
                cv2.displayOverlay(WINDOW_NAME, "Selected label: " + class_list[
                    class_index] + ""
